...
class AutoSong:
    def __init__(self, bot, channel):
        ...
        channel.set_command("R̲e̲f̲e̲r̲e̲e̲ ̲C̲o̲m̲m̲a̲n̲d̲s̲", "")
    ...
    def skip(self, message):
        if message["username"] not in self.voted:
            self.voted.append(message["username"])
            if len(self.voted) > round(len(self.channel.get_users()) / 2):
                self.carry_vote()
            else:
                self.channel.send_message(str(len(self.voted)) + " / " + str(round(len(self.channel.get_users()))) + " votes needed to skip the current beatmap")
    ...
    def on_part(self, username):
        if self.bot.format_username(username) in self.voted:
            self.voted.remove(self.bot.format_username(username))
            if len(self.voted) > round(len(self.channel.get_users()) / 2):
                self.carry_vote()
            elif self.voted:
                self.channel.send_message(str(round(len(self.channel.get_users()))) + " votes now needed to skip the current beatmap")
    ...
    def carry_vote(self):
        self.channel.send_message("Vote carried with " + str(len(self.voted)) + " votes")
        self.voted = []
