class AutoHostRotate:
    def __init__(self, bot, channel):
        ...
        self.abort_vote = channel.make_vote(self.carry_abort_vote)
        ...
        channel.set_command("!start", self.start, "If you are host, Starts the game with optional countdown timer else starts vote to begin match")
        channel.set_command("!aborttimer", channel.common_commands.abort_start_timer, "If you are a referee or host, Aborts start timer")
    ...
    def skip(self, message):
        if message["username"] == self.channel.get_formatted_host() or (message["content"] == "*skip" and self.channel.has_referee(message["username"])):
            ...
            self.skip_vote.cast_ballot(message["username"])
    ...
    def start(self, message):
        if message["username"] == self.channel.get_formatted_host():
            ...
            self.start_vote.cast_ballot(message["username"])
    ...
    def abort(self, message):
        if self.channel.in_progress():
            ...
            self.abort_vote.cast_ballot(message["username"])
